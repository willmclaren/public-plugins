...
sub createObjects {
  ...
  if ($variation) {
    ...
    my @vf                  = $self->param('vf');
    @vf                     = $self->param('vl') unless @vf && $vf[0];
    my @variation_features  = @{$variation->get_all_VariationFeatures};
    my ($variation_feature) = scalar @variation_features == 1 ? $variation_features[0] : $vf[0] ? grep {($_->dbID || $_->location_string) eq $vf[0]} @variation_features : undef;
